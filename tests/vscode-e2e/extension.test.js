...
suite('Python Language Analysis', () => {
    test('Should process Python file with imports', async function () {
        ...
        const pythonFile = vscode.Uri.joinPath(workspaceFolders[0].uri, 'tests', 'fixtures', 'python-project', 'main.py');
        try {
    ...
    test('Should analyze Python class definitions', async function () {
        ...
        const workspaceFolders = vscode.workspace.workspaceFolders;
        assert.ok(workspaceFolders && workspaceFolders.length > 0, 'Should have a workspace');
        const pythonFile = vscode.Uri.joinPath(workspaceFolders[0].uri, 'tests', 'fixtures', 'python-project', 'classes.py');
        try {
            const doc = await vscode.workspace.openTextDocument(pythonFile);
            await vscode.window.showTextDocument(doc);
            await new Promise(resolve => setTimeout(resolve, 2000));
    ...
    test('Should handle Python async/await constructs', async function () {
        ...
        const workspaceFolders = vscode.workspace.workspaceFolders;
        assert.ok(workspaceFolders && workspaceFolders.length > 0, 'Should have a workspace');
        const pythonFile = vscode.Uri.joinPath(workspaceFolders[0].uri, 'tests', 'fixtures', 'python-project', 'async_functions.py');
        try {
            const doc = await vscode.workspace.openTextDocument(pythonFile);
            await vscode.window.showTextDocument(doc);
            await new Promise(resolve => setTimeout(resolve, 2000));
    ...
    test('Should analyze Python relative imports', async function () {
        ...
        const workspaceFolders = vscode.workspace.workspaceFolders;
        assert.ok(workspaceFolders && workspaceFolders.length > 0, 'Should have a workspace');
        const pythonFile = vscode.Uri.joinPath(workspaceFolders[0].uri, 'tests', 'fixtures', 'python-project', 'relative_imports.py');
        try {
            const doc = await vscode.workspace.openTextDocument(pythonFile);
            await vscode.window.showTextDocument(doc);
            await new Promise(resolve => setTimeout(resolve, 2000));
    ...
    test('Should analyze Python decorator patterns', async function () {
        ...
        const workspaceFolders = vscode.workspace.workspaceFolders;
        assert.ok(workspaceFolders && workspaceFolders.length > 0, 'Should have a workspace');
        const pythonFile = vscode.Uri.joinPath(workspaceFolders[0].uri, 'tests', 'fixtures', 'python-project', 'decorators.py');
        try {
            const doc = await vscode.workspace.openTextDocument(pythonFile);
            await vscode.window.showTextDocument(doc);
            await new Promise(resolve => setTimeout(resolve, 2000));
...
suite('Rust Language Analysis', () => {
    test('Should process Rust file with use statements', async function () {
        ...
        const rustFile = vscode.Uri.joinPath(workspaceFolders[0].uri, 'tests', 'fixtures', 'rust-integration', 'main.rs');
        try {
    ...
    test('Should analyze Rust module system', async function () {
        ...
        const workspaceFolders = vscode.workspace.workspaceFolders;
        assert.ok(workspaceFolders && workspaceFolders.length > 0, 'Should have a workspace');
        const rustModFile = vscode.Uri.joinPath(workspaceFolders[0].uri, 'tests', 'fixtures', 'rust-integration', 'utils', 'mod.rs');
        try {
    ...
    test('Should analyze Rust helpers module', async function () {
        ...
        const workspaceFolders = vscode.workspace.workspaceFolders;
        assert.ok(workspaceFolders && workspaceFolders.length > 0, 'Should have a workspace');
        const rustFile = vscode.Uri.joinPath(workspaceFolders[0].uri, 'tests', 'fixtures', 'rust-integration', 'utils', 'helpers.rs');
        try {
            const doc = await vscode.workspace.openTextDocument(rustFile);
            await vscode.window.showTextDocument(doc);
            await new Promise(resolve => setTimeout(resolve, 2000));
...
suite('Cycle Detection', () => {
    test('Should detect simple TypeScript cycle', async function () {
        ...
        const cyclicFile = vscode.Uri.joinPath(workspaceFolders[0].uri, 'tests', 'fixtures', 'cyclic-project', 'simple-cycle', 'a.ts');
        try {
    ...
    test('Should handle complex cycle scenarios', async function () {
        ...
        const workspaceFolders = vscode.workspace.workspaceFolders;
        assert.ok(workspaceFolders && workspaceFolders.length > 0, 'Should have a workspace');
        const complexCycleDir = vscode.Uri.joinPath(workspaceFolders[0].uri, 'tests', 'fixtures', 'cyclic-project', 'complex-cycle');
        try {
    ...
    test('Should refresh graph and maintain cycle detection', async function () {
        ...
        const workspaceFolders = vscode.workspace.workspaceFolders;
        assert.ok(workspaceFolders && workspaceFolders.length > 0, 'Should have a workspace');
        const cyclicFile = vscode.Uri.joinPath(workspaceFolders[0].uri, 'tests', 'fixtures', 'cyclic-project', 'simple-cycle', 'b.ts');
        try {
            const doc = await vscode.workspace.openTextDocument(cyclicFile);
            await vscode.window.showTextDocument(doc);
            await vscode.commands.executeCommand('graph-it-live.showGraph');
...
suite('Unused Node Filtering', () => {
    test('Should register unused filter commands', async () => {
        const commands = await vscode.commands.getCommands(true);
        assert.ok(commands.includes('graph-it-live.enableUnusedFilter'), 'Should have enableUnusedFilter command');
        assert.ok(commands.includes('graph-it-live.disableUnusedFilter'), 'Should have disableUnusedFilter command');
    });
    ...
    test('Should filter unused in Rust project', async function () {
        ...
        const workspaceFolders = vscode.workspace.workspaceFolders;
        assert.ok(workspaceFolders && workspaceFolders.length > 0, 'Should have a workspace');
        const rustUnusedFile = vscode.Uri.joinPath(workspaceFolders[0].uri, 'tests', 'fixtures', 'rust-unused-deps', 'unused.rs');
        try {
...
suite('Cross-Platform Path Handling', () => {
    ...
    test('Should handle URI paths correctly', async function () {
        ...
        const uri = vscode.Uri.joinPath(workspaceFolders[0].uri, 'tests', 'fixtures');
