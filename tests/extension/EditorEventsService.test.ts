...
const saveListeners: Array<(doc: { fileName: string }) => Promise<void>> = [];
...
vi.mock('vscode', () => {
  return {
    workspace: {
      ...
      onDidSaveTextDocument: vi.fn((listener: (doc: { fileName: string }) => Promise<void>) => {
...
describe('EditorEventsService', () => {
  ...
  it('wires configuration changes to provider and MCP notifier', async () => {
    ...
    const service = new EditorEventsService({ provider, logger });
    service.register();
  ...
  it('invokes active editor and save handlers', async () => {
    const provider = createProviderMock();
    const logger = createLoggerMock();
    const service = new EditorEventsService({ provider, logger });
    service.register();
    ...
    await saveListeners[0]({ fileName: 'file.ts' });
    expect(provider.onFileSaved as ReturnType<typeof vi.fn>).toHaveBeenCalledWith('file.ts');
    expect(logger.debug).toHaveBeenCalled();
  });
});
