...
const SymbolCardView: React.FC<SymbolCardViewProps> = ({
    ...
    const getExternalDeps = (symbolId: string): Map<string, SymbolDependency[]> => {
        ...
        <div style={{
            ...
            <div style={{
                ...
                {Object.entries(CATEGORY_COLORS).map(([category, colors]) => {
                    if (!showTypes && (category === 'type' || category === 'interface')) return null;
                    return (
...
const ImporterRow: React.FC<ImporterRowProps> = ({ filePath, onNavigateToFile }) => {
    ...
    return (
        <div style={{
            ...
            <div //NOSONAR
                ...
                onKeyDown={e => handleKeyDown(e, () => onSymbolClick(symbol.id, symbol.line))}
                style={{
                    display: 'flex',
                    alignItems: 'center',
                    gap: 8,
                    ...
                    {members.map(member => {
                        ...
                        return (
                            <div //NOSONAR
                                ...
                                onKeyDown={e => handleKeyDown(e, () => onSymbolClick(member.id, member.line))}
                                style={{
                                    display: 'flex',
                                    alignItems: 'center',
                                    gap: 6,
                            ...
                            >
                                ...
                                <span style={{ fontSize: 12, color: 'var(--vscode-foreground)' }}>
                                    {memberName}
                                </span>
                                <span style={{
                                    fontSize: 10,
            ...
            {externalDeps.size > 0 && (
                <div style={{
                    ...
                    <div style={{
                        ...
                        Uses ({Array.from(externalDeps.values()).reduce((a, b) => a + b.length, 0)})
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                        {Array.from(externalDeps.entries()).map(([targetFile, deps]) => (
                            <DependencyLink
                                ...
                                symbolNames={deps.map(d => d.targetSymbolId.split(':').pop() || '')}
                                onNavigateToFile={onNavigateToFile}
                            />
                        ))}
                    </div>
...
const DependencyLink: React.FC<DependencyLinkProps> = ({
    ...
    return (
        ...
        >
            <span style={{ color: 'var(--vscode-textLink-foreground)', fontSize: 12 }}>↗</span>
            <span style={{
                fontSize: 11,
