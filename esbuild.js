...
async function main() {
  ...
  const ctxExtension = await esbuild.context({
    ...
    external: ['vscode', 'tree-sitter', 'tree-sitter-python', 'tree-sitter-rust', 'ts-morph', 'typescript'],
    logLevel: 'silent',
  ...
  const ctxWorker = await esbuild.context({
    ...
    external: ['tree-sitter', 'tree-sitter-python', 'tree-sitter-rust', 'ts-morph', 'typescript'],
    logLevel: 'silent',
    plugins: [esbuildProblemMatcherPlugin],
    target: 'node20',
    metafile: !!metafilePath,
