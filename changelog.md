...
- **`QueryEngine` provider tests no longer depend on the machine**: The test asserting a heuristic fallback with no provider configured cleared only the API-key variables, not `GRAPH_IT_LLM_PROVIDER`, which pins a provider ahead of any key check, so it failed whenever a developer had that variable exported. Provider precedence is now covered by two tests that pin `anthropic` and `openai-compatible` against fake keys: both report availability from the key alone, so nothing depends on a binary being installed or on a network call.
...
- External Review Gate distribution: the composite Action installs the published `@magic5644/graph-it-live` CLI in an isolated temporary prefix rather than building the consumer checkout. It defaults to npm `latest`, supports optional `cli-version` pinning, exposes the resolved CLI version, and rejects versions below `1.13.0`.

### Release notes
- Publishing npm `1.13.0`, verifying public `npx`, and creating the immutable `v1.13.0` Action tag are manual post-merge release tasks; this change does not publish or tag.
...
- **WASM parser path resolution in CLI**: Fixed non-TypeScript language parsers (Python, Rust, Go, Java, C#) silently falling back to `process.cwd()` when locating `dist/wasm/*.wasm` files. `extensionPath` is now computed once from `__dirname` and forwarded consistently to `AstWorkerHost` and `SpiderBuilder`, restoring multi-language analysis when the CLI is invoked from any directory.
...
- **REPL e2e test refactor**: Introduced `runWithRuntimeStub` helper to streamline REPL command execution in tests, replacing direct `run` calls for improved type safety and clarity.
