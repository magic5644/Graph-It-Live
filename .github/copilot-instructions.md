...
Keep context lean: do not load every rule preemptively. For broad architecture questions, use Graph-It-Live MCP tools first, or `graph-it` after `graph-it scan`. For edits, read exact conditional rule named by `AGENTS.md`, then inspect relevant source.
